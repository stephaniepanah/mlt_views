...
select count(*), min(sample_year), max(sample_year) from Test_DL637F where sample_year not in ('1960','1966');
-- count    minYr   maxYr
-- 115	    2001	2020



select * from Test_DL637F;
...
select wl800.sample_id, wl800.sample_year, wl800.stockpile1_pct_used, wl800.stockpile2_pct_used, wl800.stockpile3_pct_used
 ...
 where wl800.stockpile2_pct_used > 0
   and wl800.sample_id = dl637f.sample_id
...
/***********************************************************************************
 ...
 DL637-f Batch Weights for Humphres Compaction, WL360 - Fine
...
select * from V_DL637f_Batch_Weights_Humphres_Fine
;



select * from V_DL637f_Batch_Weights_Humphres_Fine where sample_id  = 'W-20-0314'
;



create or replace view V_DL637f_Batch_Weights_Humphres_Fine as

select  dl637f.sample_id as sample_id
       ,dl637f.sample_year
       ,dl637f.test_status
       ,dl637f.tested_by
       
       ,case when to_char(dl637f.dt_tested, 'yyyy') = '1959' then ' '
             else to_char(dl637f.dt_tested, 'mm/dd/yyyy')
            end as dt_tested
            
       --,dl637f.dt_tested as dt_tested_DATE
       --,dl637f.dt_tested_orig
       ...
       ,dl637f.gradation_source
       
       ,dl637f.Proposed_Batch_Weight -- not sure the source of Proposed_Batch_Weight, CHECKTHIS

       ,dl637f.exclude_stockpile1
       ,dl637f.exclude_stockpile2
       ,dl637f.exclude_stockpile3
       ,dl637f.exclude_stockpile4
       ,dl637f.exclude_stockpile5
       ,dl637f.exclude_stockpile6
  ...
  join Test_DL637F               dl637f on dl637f.sample_id = smpl.sample_id
...
/***********************************************************************************
 ...
 DL637-f Sieves grid (front page, not a segment)
 
 the WL800 stockpile previous.segment pct passing - this.segment pct passing
 becomes the pct retained on DL637-f for this.segment
...
select * from V_DL637f_Batch_Weights_Humphres_gradation_source_grid 
 where sample_id  = 'W-20-0984-AG'
 --'W-01-0130-AG'
--'W-20-0314'
;
...
create or replace view V_DL637f_Batch_Weights_Humphres_gradation_source_grid as
...
select * from V_DL637f_Batch_Weights_Humphres_Sieve_batches
 where sample_id = 
 'W-12-0492-AG'
 --'W-17-1881-AG'
 --'W-11-0157-AG'
...
create or replace view V_DL637f_Batch_Weights_Humphres_Sieve_batches as
...
with cumulative_sql as (
     ...
     select  sample_id as sampleid
       ...
       from Test_DL637F_segments
      group by sample_id, batch_nbr
)

select  dl637f_seg.sample_id   -- key
       ,dl637f_seg.batch_nbr   -- key
       ,dl637f_seg.segment_nbr -- key
       
       ,dl637f_seg.batch_description
       ,dl637f_seg.batch_weight
       ,dl637f_seg.sieve_size
       
       /*-------------------------------
         stockpile 1
       -------------------------------*/
  
       ,dl637f_seg.s1_batchwt
       ,dl637f_seg.s1_cumwt -- user entered, may get rid of this
       
       ,sum(case when dl637f_seg.s1_batchwt > 0 then dl637f_seg.s1_batchwt else 0 end)
        over (partition by dl637f_seg.sample_id, dl637f_seg.batch_nbr
        order by dl637f_seg.sample_id, dl637f_seg.batch_nbr, dl637f_seg.segment_nbr)
       ...
       ,dl637f_seg.s2_batchwt
       ,dl637f_seg.s2_cumwt
       
       ,case when dl637f_seg.s2_batchwt >= 0
             then sum(dl637f_seg.s2_batchwt) over (partition by dl637f_seg.sample_id, dl637f_seg.batch_nbr
                      order by dl637f_seg.sample_id, dl637f_seg.batch_nbr, dl637f_seg.segment_nbr)
                  + S1_calc_sum
             else -1
       ...
       ,dl637f_seg.s3_batchwt
       ,dl637f_seg.s3_cumwt
       
       ,case when dl637f_seg.s3_batchwt >= 0
             then sum(dl637f_seg.s3_batchwt) over (partition by dl637f_seg.sample_id, dl637f_seg.batch_nbr
                      order by dl637f_seg.sample_id, dl637f_seg.batch_nbr, dl637f_seg.segment_nbr)
                  + S1_calc_sum
                  + S2_calc_sum
             else -1
       ...
       ,dl637f_seg.s4_batchwt
       ,dl637f_seg.s4_cumwt
       
       ,case when dl637f_seg.s4_batchwt >= 0
             then sum(dl637f_seg.s4_batchwt) over (partition by dl637f_seg.sample_id, dl637f_seg.batch_nbr
                      order by dl637f_seg.sample_id, dl637f_seg.batch_nbr, dl637f_seg.segment_nbr)
                  + S1_calc_sum
                  + S2_calc_sum
                  + S3_calc_sum
       ...
       ,dl637f_seg.s5_batchwt
       ,dl637f_seg.s5_cumwt
       
       ,case when dl637f_seg.s5_batchwt >= 0
             then sum(dl637f_seg.s5_batchwt) over (partition by dl637f_seg.sample_id, dl637f_seg.batch_nbr
                      order by dl637f_seg.sample_id, dl637f_seg.batch_nbr, dl637f_seg.segment_nbr)
                  + S1_calc_sum
                  + S2_calc_sum
                  + S3_calc_sum
       ...
       ,dl637f_seg.s6_batchwt
       ,dl637f_seg.s6_cumwt
       
       ,case when dl637f_seg.s6_batchwt >= 0
             then sum(dl637f_seg.s6_batchwt) over (partition by dl637f_seg.sample_id, dl637f_seg.batch_nbr
                      order by dl637f_seg.sample_id, dl637f_seg.batch_nbr, dl637f_seg.segment_nbr)
                  + S1_calc_sum
                  + S2_calc_sum
                  + S3_calc_sum
  ...
  from MLT_1_WL900_Sample          smpl
  join Test_DL637F               dl637f on dl637f.sample_id = smpl.sample_id
  join Test_dl637f_segments  dl637f_seg on dl637f.sample_id = dl637f_seg.sample_id
  
  join cumulative_sql                   on dl637f_seg.sample_id = cumulative_sql.sampleid
                                       and dl637f_seg.batch_nbr = cumulative_sql.batchnbr
 
 order by smpl.sample_year desc
          ,dl637f_seg.sample_id
          ,dl637f_seg.batch_nbr
          ,dl637f_seg.segment_nbr
 ;
