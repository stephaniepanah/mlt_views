...
select * from V_T255_Moisture_Aggregate
 order by T255_Sample_Year desc, T255_Sample_ID
;
...
create or replace view V_T255_Moisture_Aggregate as 

select  t255.sample_id                         as T255_Sample_ID
       ,t255.sample_year                       as T255_Sample_Year
       ,t255.test_status                       as T255_Test_Status
       ,t255.tested_by                         as T255_Tested_by
       
       ,case when to_char(t255.date_tested, 'yyyy') = '1959' 
             then ' '
             else to_char(t255.date_tested, 'mm/dd/yyyy')
             end                              as T255_date_tested
            
       ,t255.date_tested                        as T255_date_tested_DATE
       ,t255.date_tested_orig                   as T255_date_tested_orig
       
       /*--------------------------------------------------------------------------
         user-entered values
       --------------------------------------------------------------------------*/
       
       ,case when t255.mass_wet_aggregate >= 0 then trim(to_char(t255.mass_wet_aggregate, '99990.99')) else ' ' end as T255_mass_wet_aggregate
       ,case when t255.mass_dry_aggregate >= 0 then trim(to_char(t255.mass_dry_aggregate, '99990.99')) else ' ' end as T255_mass_dry_aggregate
       ,case when t255.mass_tare          >= 0 then trim(to_char(t255.mass_tare,          '99990.99')) else ' ' end as T255_mass_tare
       ...
       ,t255.drying_temperature as T255_drying_temperature
       ,t255.temperature_scale         as T255_temperature_scale
       
       ,t255.remarks            as T255_Remarks
  ...
  join Test_T255                               t255 on t255.sample_id = smpl.sample_id
  
  /*-------------------------------------------------------------
    calculations
  -------------------------------------------------------------*/
  
  cross apply (select case when (t255.mass_wet_aggregate >= 0) then t255.mass_wet_aggregate
                           else 0 end as calc_mass_wet  from dual) CALC_WET
  
  cross apply (select case when (t255.mass_dry_aggregate >= 0) then t255.mass_dry_aggregate
                           else 0 end as calc_mass_dry  from dual) CALC_DRY
                      
  cross apply (select case when (t255.mass_tare >= 0)          then t255.mass_tare
